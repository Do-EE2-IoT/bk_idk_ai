...
#include <stdint.h>
#include <math.h>
#define SC7A20_LSB_PER_G   16384.0f   
...
{
    if (status >= ARITHMETIC_STATUS_CHECK_MAX)
        return;
...
{
    signed short x, y, z;
    static float last_acc_mag = 0.0f;
    ...
    if (shake_info.onoff == 0)
        return;

    uint32_t shake_threshold = shake_alg_param.shake_threshold_v; // m/s2 * 100 (xem bên dưới)
    uint32_t stability_threshold = shake_alg_param.stability_threshold_v;
    uint32_t shake_check_number = shake_alg_param.shake_check_number;
    uint64_t shake_time_limit_ms = shake_alg_param.shake_time_limit_ms;
    ...
    for (unsigned int i = 0; i < count; i += 3)
    {
        ...
        /* ---- raw -> m/s2 ---- */
        float ax = ((float)x / SC7A20_LSB_PER_G) * G_IN_MS2;
        float ay = ((float)y / SC7A20_LSB_PER_G) * G_IN_MS2;
        float az = ((float)z / SC7A20_LSB_PER_G) * G_IN_MS2;

        float acc_mag = sqrtf(ax * ax + ay * ay + az * az);
        float delta = fabsf(acc_mag - last_acc_mag);

        last_acc_mag = acc_mag;

        // os_printf(
        //     "[SHAKE] ax=%.2f ay=%.2f az=%.2f |a|=%.2f delta=%.2f\r\n",
        //     ax, ay, az, acc_mag, delta);

        /* ---- shake logic ---- */
        if (delta > (float)shake_threshold)
        {
            shake_info.num++;
        }
        else if (delta < (float)stability_threshold)
        {
            shake_info.num = 0;
        }

        if (shake_info.num >= shake_check_number)
        {
            uint64_t now = bk_aon_rtc_get_ms();

            if ((now - shake_info.last_time) >= shake_time_limit_ms)
            {
                shake_info.last_time = now;
                shake_info.num = 0;

                if (arithmetic_s_callback[ARITHMETIC_SHAKE_CHECK].a_s_cb)
                {
                    arithmetic_s_callback[ARITHMETIC_SHAKE_CHECK].a_s_cb(
                        arithmetic_s_callback[ARITHMETIC_SHAKE_CHECK].arg,
                        xyz_buf);
                }
            }
            break;
...
void calculate_run_steps(calculate_struct *cal_axis, signed short value)
{
    ...
    // 比较宽度是否达标，比较频率是否达标
    if (((cal_axis->cur_pos - cal_axis->old_pos) > cal_axis->MAX_TIME_TWO_POINT) || (cal_axis->dir_num > cal_axis->DIR_LINE_NUM)) // 如果高频波连续出现一定次数，则说明抖动非常剧烈，可以中止计步了
    {
        // 连续两点超过一定的时长，则计步停止
    ...
    if (cal_axis->up_down == 0) // 0: down, 1: up.
    {
        ...
        else
        {
            ...
            if (++cal_axis->dir_change > cal_axis->DIR_CHANGE_N)
            {
                ...
                {
                    // 且最大值与最小值的平均值要大于临界值
                    if ((cal_axis->dir_num < cal_axis->DIR_LINE_NUM) && (average_value > cal_axis->AVERAGE_VALUE) && (ABS(cal_axis->last_max_value) > run_cal_Sum.MIN_VALUE) &&
    ...
    if (cal_axis->up_down == 0) // 0: down, 1: up.
    {
        ...
        else
        {
            ...
            if (++cal_axis->dir_change > cal_axis->DIR_CHANGE_N)
            {
                ...
                {
                    // 且最大值与最小值的平均值要大于临界值
                    if ((cal_axis->dir_num < cal_axis->DIR_LINE_NUM) && (average_value > cal_axis->AVERAGE_VALUE) && (ABS(cal_axis->last_max_value) > cal_Sum.MIN_VALUE) &&
